...
<script>
import { getProfile,uploadAvatar,resetPassword } from '@/api/system/profile'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      imageUrl: '',
      uploadUrl: process.env.BASE_API + '/admin/avatar/upload',
    }
  },
  methods:{
    onSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          resetPassword(this.saveForm).then(res => {
            this.$message({
              message: '保存成功',
              type: 'success'
            });
    ...
    saveAvatar() {
      ...
      uploadAvatar(this.imageUrl).then(res => {
        this.$message({
          message: '保存成功',
          type: 'success'
        });
    ...
    handleAvatarSuccess(res, file) {
      if (res.resp_code === 200) {
        this.imageUrl = res.result.img_path;
      }
    },
