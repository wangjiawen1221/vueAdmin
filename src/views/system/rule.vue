        ...
        <el-row style="padding-top: 10px;">
          ...
          >
                        <span class="custom-tree-node" slot-scope="{ node, data }">
                            ...
                            <span>
                                <el-button type="text" size="mini" @click="() => editRule(data)" v-if="">编辑</el-button>
                                <el-button type="text" size="mini" @click="() => deleteRule(data)" v-if="">删除</el-button>
                            </span>
                        </span>
          </el-tree>
        </el-row>
...
<script>
import { getRouterList,getMenusList,getRouterById,deleteRouterById,saveForm } from '@/api/system/rule'
import { getRulesList } from '@/api/system/group'
...
export default {
  ...
  methods:{
    ...
    getRoutes() {
      getRouterList(this.form).then( res =>{
          this.routes = res.map(function (item) {
            return {value:item};
          });
      });
    },
    getMenusTree() {
      getMenusList(1,1).then(res =>{
          this.menusTree = res;
      });
    },
    getRulesTree() {
      getRulesList(1).then(res =>{
        this.rulesTree = res;
    ...
    editRule(node) {
      if(typeof(this.$refs['saveForm']) !== 'undefined') this.$refs['saveForm'].resetFields();
      getRouterById(node.id).then(res=>{
        this.saveForm = res;
        this.saveForm.status = res.status === 1;
        this.saveForm.menu += '';
    ...
    deleteRule(node) {
      this.$confirm('你确定删除这条记录吗?', '提示', {
        ...
        deleteRouterById(node.id).then(res =>{
            if(res == 'success') {
              this.$message({
                message: '删除成功',
                type: 'success'
